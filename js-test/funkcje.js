...
function oblicz_kwote_laczna(waluta, pozycja_symbolu){
    ...
    $('td.pozycja_cena').each(function(ind, el){
		...
		if($(el).closest('tr').next().find('.checkBox').prop("checked")){
		//if($(el).parents('tr').next('tr').find(".checkBox").prop("checked")){}
		powloka=parseFloat($(el).closest('tr').next().find('.powloka-hidden').attr('pow-sum'));
		}
		else{
		powloka=0;
    ...
    if(rabat_r != 0){
		...
		if(hurt > 0 && ilosc >= hurt_ilosc){
			...
			if(rodzaj_r == 'p'){
				var rabat_procentowy = suma/100*rabat_r;
				suma -= rabat_procentowy;
				$('.rabat-text').removeClass('ukryj');
				$('.rabat').empty().append('('+rabat_r+'%) ').append(dodaj_walute(rabat_procentowy.toFixed(2), waluta, pozycja_symbolu));	
			}

			if(rodzaj_r == 'k'){
				suma -= rabat_r;
				$('.rabat-text').removeClass('ukryj');
				$('.rabat').empty().append(dodaj_walute(rabat_r.toFixed(2), waluta, pozycja_symbolu));
				}
			}
    }else{
		$('.rabat').empty();
...
$('.koszyk-tabela-poz-cal-platnosci-poz').click(function(){
$(this).css('border', '');
$('.karta-error').addClass('ukryj');
//$('.koszyk-tabela-poz-cal-platnosci-poz').style.removeProperty('border');
...
/*clear button PODSUMOVANIE*/
function ClearPodsumowanieButton(){
	$(".potwierdz_podsumowanie").removeAttr("disabled", "disabled");
	$(".potwierdz_podsumowanie").removeAttr("pointer-events", "none");
	$(".potwierdz_podsumowanie").removeAttr("cursor", "default");
	$(".potwierdz_podsumowanie").removeAttr("style", "background-color: #B1B6BA;background-image: none");
}
/*end clear button PODSUMOVANIE**/
...
$('.potwierdz_podsumowanie').click(function(e){
	...
	var lang=$('.wybierzjezyk div').attr('data-lang');	

	var numer_karty=$('#nrk').val();//numer karty
	var name_card=$('#wk').val();//imie
	var code_card=$('#kwk').val();//kod weryfikacyjny
	var mies=$('#mies').val();//miesac
	var rok=$('#rok').val();//rok
	...
	if((numer_karty=="" || name_card=="" || code_card=="" || mies==null || rok==null) && $('#platnosc').is(':checked')){
		ok=false;
		$('.karta-error').removeClass('ukryj').text(uzupelnijwszystkiepolaformularza); 
	    //	$(".potwierdz_podsumowanie").append().text(proszejeszczeraz);
	    $(".potwierdz_podsumowanie").text(proszejeszczeraz);
	    $('.koszyk-tabela-poz-cal-platnosci-poz').css('border', 'solid thin red');	
}
	...
	if(disab != "disabled"){
		...
		typ_platnosci=2;
			$.post("./ajax/koszyk.html", {akcja: "wyslij_zamowienie", daneosobowe:daneosobowe, kwota_laczna:kwota_laczna, rabat:rabat, koszt_wysylki:koszt_wysylki}).done(function(){
		    	$.post("./ajax/mail.html", {akcja: "zlozenie", id_zam: id_zam, typ_platnosci:typ_platnosci}).done(function(datam){
		    	});
	    ...
	    if($('#platnosc').is(':checked')){	
	    	typ_platnosci=1;
					...
					$.post("./ajax/koszyk.html", {akcja: "paylane", kwota_laczna: kwota_laczna, numer_karty:numer_karty, name_card:name_card, code_card:code_card,
							mies:mies, rok:rok}).done(function(data){	

								if(data.trim()=="no"){
									$('.karta-error').removeClass('ukryj').text(platnoscamericanexpressniedostepna); 
									$(".potwierdz_podsumowanie").attr("readonly", "readonly");					
	    							//ClearPodsumowanieButton();
	    							//$(".potwierdz_podsumowanie").text(proszejeszczeraz);
	    							console.log('zapłata przez American Express nie wylonana');
								}
								if(data.trim()=="yes"){								
									$.post("./payment/paylane.html", {kwota_laczna: kwota_laczna, rabat:rabat, koszt_wysylki:koszt_wysylki, numer_karty:numer_karty}).done(function(data){
										if(data.trim() == "ok"){
												$.post("./ajax/mail.html", {akcja: "zlozenie", id_zam: id_zam, typ_platnosci:typ_platnosci}).done(function(datam){	
													console.log(datam);
												});	
			    								window.location.href = 'https://'+_serwer+_sciezka+'/zamowienie.html';
			    								console.log('zapłata udana');
			    								$.post("./ajax/koszyk.html", {akcja: "koniec_zamowienia"});
										}
										else{
			    								//window.location.href = 'https://'+_serwer+_sciezka+'/podsumowanie.html';
                            					$('.karta-error').removeClass('ukryj').empty().append("Zaplata nieudana: ");
                            					$('.karta-error').append(data);
												console.log('zapłata nieudana powrót do podsumowania');
												ClearPodsumowanieButton();
	    										$(".potwierdz_podsumowanie").text(proszejeszczeraz);
															 		}
...
$('.dalej_dane_klienta').click(function(e){
	...
	var imie_odb = $('#imie_2').val();
    var nazwisko_odb = $('#nazwisko_2').val();
    var firma_odb = $('#firma_2').val();
    var email_odb = $('#email_2').val();
    var adres_odb = $('#adres_2').val();
    var adres2_odb = $('#adres2_2').val();
    var kod_odb = $('#kod_2').val();
    var miejsc_odb = $('#miejscowosc_2').val();
    var tel_odb = $('#tel_2').val();
    var kraj_odb = $('#kraj_wysylka').attr('data-id');
...
*/
	...
	var option1_checked=$('#zam1').is(':checked');
	var option2_checked=$('#zam2').is(':checked');
    ...
    if($('#woj_2').length > 0){
		var woj_odb = $('#woj_2').find('option:selected', this).attr('data-id');
    }else{
		var woj_odb = 0;
    }
    ...
    var tekst_numer = $('#form_dane_klienta').attr('data-uwaga-numer');
    var ok = true;
    var ok_odb = true;
	var byl_skok = false;
	...
	$('#imie').attr('data-blad', "0");
 	...
 	if($('#imie_2').val() == ''&& option2_checked){
	ok_odb = false;
	$('#imie_2').parent().parent().find('span').empty().append(tekst);
	$('#imie_2').attr('data-blad', "1");
	do_bledu('#imie_2');
    }else{
	$('#imie_2').parent().parent().find('span').empty().append('*');
	$('#imie_2').attr('data-blad', "0");
    }
console.log(ok_odb);
console.log(ok);
    ...
    if($('#nazwisko_2').val() == '' && option2_checked){
	ok_odb = false;
	$('#nazwisko_2').parent().parent().find('span').empty().append(tekst);
	$('#nazwisko_2').attr('data-blad', "1");
	do_bledu('#nazwisko_2');
		
	}else{
	$('#nazwisko_2').attr('data-blad', "0");
	$('#nazwisko_2').parent().parent().find('span').empty().append('*');
    }
console.log(ok_odb);
console.log(ok);
    ...
    if(email == '' || !email_ok(email)){
			...
			if(email == ''){
				...
				console.log('pola email jest puste');
					if(!byl_skok){
						do_bledu('#email');
						byl_skok = true;
					}
			...
			if(!email_ok(email)){
				$('#email').parent().parent().find('span').empty().append("<span class='span-error'>"+tekst_numer+"</span");
				$('#email').attr('data-blad', "1");	
				console.log('nieprawidlowy format pola email');
		}
	ok = false;
	...
	$('#email').parent().parent().find('span').empty().append('*');
 	...
 	if(($('#email_2').val() == '' || !email_ok($('#email_2').val())) && option2_checked){
		if (($('#email_2').val() == '')){
			$('#email_2').parent().parent().find('span').empty().append(tekst);
			$('#email_2').attr('data-blad', "1");
			do_bledu('#email_2');
		}
		if(!email_ok($('#email_2').val())){
				$('#email_2').parent().parent().find('span').empty().append("<span class='span-error'>"+tekst_numer+"</span");
				$('#email_2').attr('data-blad', "1");	
			}
	ok_odb = false;
    }else{
	$('#email_2').attr('data-blad', "0");
	$('#email_2').parent().parent().find('span').empty().append('*');
    }
console.log('email2 - '+ok_odb);
console.log('email - '+ok);
    ...
    if($('#adres_2').val() == '' && option2_checked){
	ok_odb = false;
	$('#adres_2').parent().parent().find('span').empty().append(tekst);
	$('#adres_2').attr('data-blad', "1");
	do_bledu('#adres_2');
    }else{
	$('#adres_2').parent().parent().find('span').empty().append('*');
	$('#adres_2').attr('data-blad', "0");
    }
    console.log('Adres2 - '+ok_odb);
...
var parsedKod=kod.replace(/\D+/g,"");
    if(kod == '' || parsedKod.length<1){
    	if(parsedKod.length<1){
    		$('#kod').parent().parent().find('span').empty().append("<span class='span-error'>"+tekst_numer+"</span");
...
var parsedKod_2=kod_odb.replace(/\D+/g,"");
    if((kod_odb == ''|| parsedKod_2.length<1) && option2_checked){

    	if(parsedKod_2.length<1){
    		$('#kod_2').parent().parent().find('span').empty().append("<span class='span-error'>"+tekst_numer+"</span");
			$('#kod_2').attr('data-blad', "1");
    	}

    	if(kod_odb == ''){
			$('#kod_2').parent().parent().find('span').empty().append(tekst);
			$('#kod_2').attr('data-blad', "1");
					if(!byl_skok){
						do_bledu('#kod_2');
						byl_skok = true;
	...
	$('#kod_2').attr('data-blad', "0");
	$('#kod_2').parent().parent().find('span').empty().append('*');
    }
    console.log('kod_2 - '+ok_odb);
    console.log('kod - '+ok);
    ...
    if($('#miejscowosc_2').val() == '' && option2_checked){
	ok_odb = false;
	$('#miejscowosc_2').parent().parent().find('span').empty().append(tekst);
	$('#miejscowosc_2').attr('data-blad', "1");
	do_bledu('#miejscowosc_2');
    }else{
	$('#miejscowosc_2').attr('data-blad', "0");
	$('#miejscowosc_2').parent().parent().find('span').empty().append('*');
    }

console.log('#miejscowosc2-'+ok_odb);
console.log('#miejscowosc-'+ok);
    ...
    if(woj_odb == 0 && $('#woj_2').length > 0 && option2_checked){
	ok_odb = false;
	$('#woj_2').parent().parent().find('span').empty().append(tekst);
	do_bledu('#woj_2');
    }else{
	$('#woj_2').parent().parent().find('span').empty().append('*');
    }

console.log('woj2 - ' + ok_odb);
//console.log('woj' + woj);
console.log('woj - ' + ok);
...
console.log(ok);
    ...
    if($('#tel_2').val() == '' && option2_checked){
	ok_odb = false;
	$('#tel_2').parent().parent().find('span').empty().append(tekst);
	$('#tel_2').attr('data-blad', "1");
	do_bledu('#tel');
	}else{
	$('#tel_2').attr('data-blad', "0");
	$('#tel_2').parent().parent().find('span').empty().append('*');
    }
console.log(ok_odb);
console.log(ok);
...
console.log('final result - '+ok_odb);
//ok_odb=true;
    if(ok&&ok_odb){
    	console.log(' checkbox 1');
			...
			$.post("./ajax/koszyk.html", {akcja: "zapisz_dane_klienta", imie:imie, nazwisko:nazwisko, firma:firma, email:email, adres:adres, adres2:adres2, kod: kod, tel:tel, kraj: kraj, woj:woj, miasto:miasto, nagranie:nagranie, imie_odb:imie_odb, nazwisko_odb:nazwisko_odb, firma_odb:firma_odb, email_odb:email_odb, adres_odb:adres_odb, adres2_odb:adres2_odb, kod_odb:kod_odb, tel_odb:tel_odb, kraj_odb:kraj_odb, miejsc_odb:miejsc_odb, woj_odb:woj_odb, adres_wysylki:adres_wysylki}).done(function(data){
...
$('.zapisz_dane_wysylki').click(function(e){
	...
	$.post("./ajax/koszyk.html", {akcja: "zapisz_dane_wysylki", adres_wysylki:adres_wysylki, typ_platnosci:typ_platnosci, imie:imie, nazwisko:nazwisko, firma:firma, email:email, adres:adres, adres2:adres2, kod: kod, tel:tel, kraj: kraj, woj:woj, miasto:miasto}).done(function(data){
		//alert(data);
	    window.location.href = 'https://'+_serwer+_sciezka+'/podsumowanie.html';
	});
    }
...
$('.blad-kod').click(function(){
    $(this).addClass('ukryj');
})
$('.wpisany-kod').click(function(){
    $(this).addClass('ukryj');
})
$('.brak-kod').click(function(){
    $(this).addClass('ukryj');
})
$('.ok-kod').click(function(){
    $(this).addClass('ukryj');
})
...
$('.zastosuj').click(function(e){
    ...
    var kraj = $('.koszyk-teren-kraju').attr('data-id');
    var kod = $('.input-kod').val();
    
	if(kod==0){
	$('.brak-kod').removeClass('ukryj');
	}
	else{
	...
	console.log(data);
	if(data == 'blad'){
		$(".blad-kod").removeClass('ukryj');	

	}else if(data == 'wpisany'){
		$(".wpisany-kod").removeClass('ukryj');	  

	}else{
		//$(".ok-kod").removeClass('ukryj');
		$('.wpisz-rabat').addClass("ukryj",300);
		$('.kupon-rabat-top').remove();
    ...
    $('.wybierzjezyk div').click(function(){
	...
	$.post("./ajax/koszyk.html", {akcja: "czysc_koszyk"}).done(function(data){
		...
		$('.wybierzjezyk div').each(function(ind, el){
...
$('.input_wysylka').on('focus', function(){
     $(this).next().children().attr('style', 'color:white');
     $(this).next().children().empty().append('*');
	//$(this).parent().find('span').empty().append('*');
});
...
$('.input_wysylka').on('blur', function(){
	var tekst = $('#form_wysylka').attr('data-uwaga');
	if($(this).val() == ''){
		$(this).next().children().empty().append(tekst);
		$(this).next().children().attr('style', 'color:#e84e50');
	}
});
...
function plusminus()
{
  ...
  $( ".plus" ).click(function(e) {
	...
	var price_powloka = $('.checkBox').closest("td").next().next().attr('data-koszt')
	var price_powloka_box=$(this).parents('tr').next('tr').find('.powloka-hidden');	
	//if($(this).parents('tr').next('tr').find(".checkBox").prop("checked")){
		powloka_sum=(price_powloka*ileplusminus).toFixed(2);
 		$(price_powloka_box).empty().append(dodaj_walute(powloka_sum, waluta, pozycja_symbolu));
 		$(price_powloka_box).attr('pow-sum', powloka_sum);
...
/*Start handling checkbox-powloka*/
$('.checkBox').click(function() {
var klucz = $(this).closest("tr").prev().attr('data-klucz');

if($(this).prop("checked")){
			var powloka=1;
        	$.post("./ajax/koszyk.html", {akcja: "powloka", powloka: powloka, klucz: klucz});
        	$(this).closest("td").next().next().css({"color": "#464648"});
        	ileplusminus = $(this).closest('tr').prev().children('td').children('div').children('input').val();
        	console.log(ileplusminus); 
        	powloka_sum = ($(this).closest("td").next().next().attr('data-koszt')*ileplusminus).toFixed(2); 
			$(this).closest("td").next().next().empty().append(dodaj_walute(powloka_sum, waluta, pozycja_symbolu));
			$(this).closest("td").next().next().attr('pow-sum', powloka_sum);
        }
        else{
        	var powloka=0;
        	$(this).closest("td").next().next().css({"color": "lightgray"});
        	$(this).closest("td").next().next().attr('pow-sum',0);
        }
...
/*End handling checkbox-powloka*/

//$('.powloka-hidden').css({"color": "lightgray"});
plusminus();
...
function wybierz()
{
    $('body .f-wybierz ul div').each(function( index ) {
        atrwyb =  $(this).attr('selected');
        if(atrwyb == 'selected')
        {
        	$("#f-image").remove();
           atrsrc = $(this).attr('data-lang');
           
           $('body .f-wybierz .f-wybierz-s').prepend('<div id="f-image" class="flagi flaga-'+atrsrc+'"></div>');
        }
    });
    $('body .f-wybierz ul div').click(function() {
       $("#f-image").remove();
       atrsrc = $(this).attr('data-lang');
       $('body .f-wybierz .f-wybierz-s').prepend('<div id="f-image" class="flagi flaga-'+atrsrc+'"></div>');
...
//obsluga 'copowiesz na 10% rabatu'
$(".wyslijmailrabatsubmit").click(function(e){
e.preventDefault();
...
var ok=true;
	if($('#emailkontakt').val() == '' || !email_ok(email)){
		...
		ok=false;
		if(!email_ok(email)){
				var tekst_numer = $('#wyslijmail').attr('data-uwaga-numer');
				$('#emailkontakt').parent().find('span').empty().append("<span class='span-error'>"+tekst_numer+"</span");
	...
	if(ok){
			$.post("./ajax/mail.html", {akcja:"walidacja_email", email:email}).done(function(data){
				console.log(data);
			if(data.trim()==1){
				$.post("./ajax/mail.html", {akcja:"rabat_wysylka", email:email}).done(function(data_wysylka){
					console.log(data_wysylka);
					if(data_wysylka.trim()=='ok'){
						$('.znizka').fadeOut(600);
						$('.znizka2').fadeIn(500);
					}else{
						$('.znizka h3').show();
